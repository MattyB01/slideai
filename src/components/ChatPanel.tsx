...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3`}>
      <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${isUser ? 'bg-blue-600 text-white rounded-br-md' : 'bg-zinc-100 text-zinc-800 rounded-bl-md'}`}>
        <p className="whitespace-pre-wrap break-words">{message.content}</p>
        <span className={`block mt-1 text-[10px] ${isUser ? 'text-blue-200' : 'text-zinc-400'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3">
        {emptyState && (
          <div className="flex flex-col items-center justify-center h-full text-center px-2">
            ...
            <p className="text-sm font-medium text-zinc-500 mb-1">How can I help?</p>
            <p className="text-xs text-zinc-400 leading-relaxed">
              {scope === 'slide' ? 'Edit the currently selected slide.' : 'Modify the entire presentation.'}
            </p>
          </div>
        )}
      ...
      {emptyState && (
        <div className="px-3 pb-2 shrink-0">
          <div className="flex flex-wrap gap-1.5">
            {quickActions.map((qa) => (
              <button key={qa.action} onClick={() => handleQuickAction(qa.action)} className="text-[11px] font-medium px-2.5 py-1.5 rounded-lg bg-zinc-100 text-zinc-600 hover:bg-zinc-200 hover:text-zinc-800 transition-colors duration-150">{qa.label}</button>
            ))}
          </div>
        </div>
      )}
