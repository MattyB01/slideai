...
export default function Toolbar() {
  ...
  const handleThemeSelect = async (theme: SlideTheme, presetName: string) => {
    ...
    try {
      ...
      if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Restyle failed'); }
  ...
  return (
    <header className="h-12 bg-white border-b border-zinc-200 flex items-center px-4 gap-3 shrink-0">
      ...
      <div className="relative" ref={themeRef}>
        <button onClick={() => setThemeOpen(!themeOpen)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 transition-colors duration-150">
          ...
          <div className="absolute right-0 top-full mt-1 w-72 bg-white border border-zinc-200 rounded-xl shadow-lg shadow-zinc-200/50 z-50 py-1.5">
            ...
            {themePresets.map((preset) => (
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2 text-sm text-left text-zinc-700 hover:bg-zinc-50 transition-colors duration-100"
              >
      ...
      {/* Export */}
      <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium bg-zinc-900 text-white hover:bg-zinc-800 active:bg-zinc-700 transition-colors duration-150">
